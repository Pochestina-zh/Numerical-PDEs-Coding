%% 迎风格式求解传输方程（a=1,显示右偏心）
clear;
clc;

%网格参数
J=1000;       %空间网格数,范围[0,2pi]
K=5*J;      %时间网格数,时间范围为[0,pi]
h=2*pi/J;   %空间步长
tau=0.1*h;  %时间步长
r=0.1;      %网格比
...
U=zeros(5*J+1,K+1);   %表示增广数值解，范围[0,10pi]
...
%初始条件
for j=1:J+1
    U(j,1)=sin((j-1)*h);               %t=0，初始下边界[0,2pi]
end
%周期性边界条件
for i=1:4
    for j=1:J
        U(j+i*J,1)=U(j,1);                  %t=0的初始下边界[2pi,10pi]
    end
end
U(5*J+1,1)=U(1,1);
%% 计算每层
